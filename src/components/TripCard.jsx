...
const TripCard = ({
  ...
  return (
    <div
      className={`relative w-full lg:min-w-5xl xl:min-w-6xl ${darkMode
    ...
    >
      ...
      <div className="relative h-[300px] sm:h-[350px] md:h-[420px] lg:h-[480px] w-full flex-shrink-0 overflow-hidden rounded-t-3xl">
        <div className="absolute inset-0 overflow-hidden rounded-t-3xl bg-gray-200 dark:bg-gray-700">
          {imageError || !trip.image ? (
            ...
            >
              <ImageOff
                className={`w-24 h-24 ${darkMode ? "text-gray-500" : "text-gray-400"}`}
              />
            </div>
          ) : (
            <img
        ...
        {trip.isWishlist && (
          <div
            title="Wishlist Trip"
            className={`absolute top-5 left-5 ${darkMode ? "bg-pink-600/90 text-white" : "bg-pink-500/90 text-white"
        ...
        <div className="absolute top-5 right-5 flex gap-2 z-20">
          <button
            onClick={() => onEdit(trip)}
            title="Edit Trip"
            className={`cursor-pointer p-2.5 rounded-full backdrop-blur-lg shadow-lg transform transition-all duration-300 hover:scale-110 focus:outline-none ring-1 ${darkMode
              ? "bg-gray-800/70 text-gray-100 hover:bg-gray-700 ring-gray-600"
              : "bg-white/80 text-gray-700 hover:bg-gray-50 ring-gray-300"
              }`}
          >
          ...
          <button
            ...
            title="Delete Trip"
            className={`cursor-pointer p-2.5 rounded-full backdrop-blur-lg shadow-lg transform transition-all duration-300 hover:scale-110 focus:outline-none ring-1 ${darkMode
              ? "bg-red-900/50 text-red-300 hover:bg-red-700/70 hover:text-red-100 ring-red-700/80"
              : "bg-red-100/80 text-red-600 hover:bg-red-200/90 hover:text-red-700 ring-red-300/90"
              }`}
          >
        ...
        <div className="absolute top-1/2 left-0 right-0 flex justify-between px-3 transform -translate-y-1/2 z-10">
          <button
            ...
            className={`cursor-pointer p-3 rounded-full ${darkMode
              ? "bg-gray-900/70 text-gray-100 hover:bg-gray-800/90"
              : "bg-white/80 text-gray-800 hover:bg-white"
              } backdrop-blur-lg shadow-xl transform transition-all duration-300 hover:scale-110 focus:outline-none ring-2 ${darkMode ? "ring-gray-700" : "ring-gray-300"
        ...
        <div className="absolute bottom-4 left-4 right-4 sm:bottom-5 sm:left-5 sm:right-5 md:bottom-6 md:left-6 md:right-6 z-10">
          <div className="flex items-end justify-between gap-2 sm:gap-3">
            ...
            {/* Rating Badge */}
            <div
              className={`backdrop-blur-md rounded-lg sm:rounded-xl shadow-md sm:shadow-lg flex items-center border flex-shrink-0 px-1.5 py-1 sm:px-2 sm:py-1.5 md:px-3 md:py-2 space-x-1 sm:space-x-1.5 ${darkMode
            ...
            >
              <RatingStars
                rating={trip.rating}
                darkMode={darkMode}
                size="h-3 w-3 sm:h-4 sm:w-4 md:h-5 md:w-5"
              />
              <span
                className={`font-bold text-xs sm:text-sm ${darkMode ? "text-gray-100" : "text-gray-800"
                  }`}
              >
            ...
            <div className="flex space-x-1 sm:space-x-1.5 mt-2.5 sm:mt-3 md:mt-4 justify-center">
              {[...Array(totalTrips)].map((_, i) => (
                <button
                  ...
                  className={`cursor-pointer rounded-full transition-all duration-500 ease-out focus:outline-none h-1.5 w-1.5 sm:h-2 sm:w-2 ${i === index
                    ? "w-3 sm:w-4 bg-white shadow-sm"
      ...
      <div className="p-6 md:p-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-3">
          <div
            className={`flex items-center text-sm font-medium ${darkMode
          ...
          >
            <Calendar className="h-5 w-5 mr-2.5 opacity-80" />
            <span>{trip.date}</span>
          </div>
          {trip.tags && trip.tags.length > 0 && (
            <div className="flex flex-wrap gap-2 items-center">
              {trip.tags.slice(0, 3).map((tag, idx) => (
                <span
                  key={idx}
                  className={`text-xs font-medium px-2.5 py-1 rounded-full border ${darkMode
              ...
              {trip.tags.length > 3 && (
                <span className="text-xs text-gray-500">+{trip.tags.length - 3} more</span>
              )}
            </div>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          {/* Highlights */}
          <div
            className={`border-2 rounded-2xl p-5 transition-all duration-300 hover:shadow-lg hover:-translate-y-0.5 ${darkMode
          ...
          >
            ...
            >
              <div
                className={`p-2.5 rounded-full ${darkMode ? "bg-emerald-900/50" : "bg-emerald-100"
                  } mr-3 transition-colors duration-300 shadow-inner`}
              >
                <CheckCircle
                  className={`h-6 w-6 ${darkMode ? "text-emerald-300" : "text-emerald-500"
          ...
          >
            ...
            >
              <div
                className={`p-2.5 rounded-full ${darkMode ? "bg-rose-900/50" : "bg-rose-100"
                  } mr-3 transition-colors duration-300 shadow-inner`}
              >
                <AlertTriangle
                  className={`h-6 w-6 ${darkMode ? "text-rose-300" : "text-rose-500"
                    } transition-colors duration-300`}
                />
              </div>
              Considerations
            ...
            <ul className="space-y-2.5">
              {trip.lowlights.map((lowlight, idx) => (
                ...
                >
                  <div className="mr-3 mt-1.5 w-2 h-2 bg-rose-400 rounded-full flex-shrink-0"></div>
                  <span>{lowlight}</span>
                </li>
              ))}
            </ul>
