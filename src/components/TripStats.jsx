  ...
  return (
    <div
      className={`p-5 rounded-2xl border-2 ${
        darkMode ? `${c.darkBorder} bg-gradient-to-br ${c.darkBg}` : `${c.lightBorder} bg-gradient-to-br ${c.lightBg}`
    ...
    >
      ...
      <p className={`text-3xl font-extrabold mb-0.5 ${darkMode ? c.darkText : c.lightText}`}>{value}</p>
      <p className={`text-xs font-medium uppercase tracking-wide ${darkMode ? "text-gray-200" : `${c.lightText}/90`}`}>
        {label}
      </p>
    </div>
  );
...
const TripStats = ({ trips, darkMode }) => {
  ...
  if (visitedTrips.length === 0) {
    return (
      ...
      >
        <h3 className={`text-2xl font-bold font-playfair ${darkMode ? "text-gray-200" : "text-gray-800"} mb-3`}>
          Travel Statistics
        </h3>
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-5">
        <StatCard icon={Plane} value={visitedTrips.length} label="Trips Taken" colorName="blue" darkMode={darkMode} />
        <StatCard icon={Globe} value={totalContinents} label="Continents" colorName="purple" darkMode={darkMode} />
        <StatCard icon={MapPinned} value={totalCountries} label="Countries" colorName="pink" darkMode={darkMode} />
        <StatCard icon={Star} value={`${avgRating}/5`} label="Avg. Rating" colorName="amber" darkMode={darkMode} />
      </div>
    </div>
  );
};
